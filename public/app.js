...
function renderFounders(founders) {
  const wrap = byId("founder-grid");
  if (!wrap) return;

  wrap.innerHTML = (founders || [])
    .map(
      (founder) => `
        <article class="founder-card">
          <div class="founder-photo">
            <img src="${escapeHtml(founder.imageUrl || "")}" alt="${escapeHtml(
              founder.name,
            )}" />
          </div>
          <div class="founder-copy">
            <h4>${escapeHtml(founder.name)}</h4>
            <div class="inst">${escapeHtml(founder.role)} | ${escapeHtml(
              founder.institution,
            )}</div>
...
async function bootHome() {
  const [
    ...
    supabase.from("site_intro").select("*").eq("id", true).maybeSingle(),
  ...
  const firstError = [
    introResult.error,
    foundersResult.error,
    researchResult.error,
    questionnaireResult.error,
  ].find(Boolean);
  if (firstError) throw firstError;

  const intro = mapIntro(introResult.data);
  const founders = (foundersResult.data || []).map(mapFounder);
  ...
  byId("club-title").textContent = intro.title || "PACD Research Club";
  byId("club-background").textContent = intro.background || "";
  byId("club-source").textContent = intro.source || "";
  byId("hero-image").src =
    intro.groupImage || assetUrl("assets/home-1-logo.jpg");
  byId("club-statement").innerHTML = (intro.statement || [])
...
bootHome().catch((error) => {
  const node = byId("club-background");
  if (node) {
    node.textContent = `The site backend is not ready: ${error.message}`;
  }
  setupJoinDialog();
});
